...
class Rssrai(data.Dataset):
    ...
    def __init__(self,
        ...
        :param split: train/val or ["train","val"]
        ...
        test_csv_url = self._base_dir + '/rssrai_test.csv'
        data = pd.read_csv(train_csv_url)
        data['img'].value_counts()
        # 切分训练集和验证集
        ...
        for splt in self.split:
            ...
            for name in lines:
                _image = os.path.join(self._image_dir, name)
                _categ= os.path.join(self._cat_dir, name)
